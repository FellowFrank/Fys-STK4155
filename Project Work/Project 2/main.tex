...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\columnwidth]{figures/MSE for Different Architectures.png}
...
The results, shown in Figure \ref{fig:Training_struggle}, demonstrate that this simple configuration fails to learn the underlying Runge function. 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\columnwidth]{figures/MSE for Different Optimizers.png}
    \caption{Heatmap showing that the choice of optimization algorithm has a significant impact on the FFNN's performance in learning, trained on 1000 epochs.}
...
\begin{table}[htbp]
    \caption{The distribution of classes in the Thrash dataset.}
...
The results of this benchmark figure \ref{fig:final-comparison-regression} reveal the real story. 
...
Where we in this case will have more data to train on and therefore escape the randomness contained in comparing sparse data sets.  
...
I.e. we will try on this larger set to find a good baseline to search the hyperparameter space for the best hyper parameters for the MNIST dataset.

As the MNIST dataset is much larger than the sparse dataset, we needed to use a smaller grid search with fewer epochs.
...
Therefore we used a subset of 1000 samples from the training set for each training run.
...
Looking at the results that our naive grid search found and then continuing the training run. 
We saw that this give was a good starting point, but there was room for improvement as it had an accuracy of around 94\% and we where able to improve it further by making some adjustments.
We therefore continued to tune the parameters further, by alternating some of these parameters to see how these changes develop on a longer training run.
One specific change we saw was beneficial was a slightly lower learning rate and a larger batch size to improve the stability in learning.
After having done this final fine-tuning we ended up with the following hyperparameters in table \ref{tab:hyperparameters_mnist}.

\begin{table}[H]
    \caption{Hyperparameters used for the neural network training.}
    \label{tab:hyperparameters_mnist}
    \begin{ruledtabular} % This replaces \centering and adds the lines
    \begin{tabular}{l r}
        % No \hline\hline needed - ruledtabular does it.
        \textbf{Parameter} & \textbf{Value} \\
...
Where we see that the model is performing quite well, with a high percentage being correct predictions, i.e. the diagonal.
...
One important observation from this matrix is the mistake of classifying 5 as a 3 is the most common mistake, which is interesting as they don't look that similar.
...
As all of these looks like 5s, perhaps the middle one can be seen as a 3, and a neural network that is better at capturing spatial hierarchies might help.
In view of this there is reason to believe that there is more room for improvements to this classification. 
...
As a concluding remark, we acknowledge that despite of these remarks the FFNN model is able to perform quite well out of the box on the MNIST dataset with an accuracy of 97.51\%.
Where it has shown clear capabilities and adaptability to learn complex functions from data.


\section{further work}
Some problems that could use a closer study:
\begin{enumerate}[a)]
    \item Finding more cost efficient ways to do hyper parameter tuning.As the naive approach we used is very computationally expensive.
    \item How different architectures with regards to layers and nodes per layer affect the learning capabilities of the FFNN. 
    Are triangles better than rectangles? 
    Is it better to have one hidden activation function for all layers or to mix them? 
    \item How parallelization and GPU acceleration can speed up the training process for FFNNs implemented from scratch.
    \item How Convolutional Neural Networks (CNNs) would perform in comparison to FFNNs on image classification tasks.
\end{enumerate}
...
As the measurement was along a few epochs where the one that had the best run out of these few epochs was chosen.
A better approach would perhaps be to see how steady the improvement was over time rather than just looking at the best result after a few epochs, perhaps by measuring slope and variance over time.
...
Where Adam and RMSprop was needed in order to get good results, as plain GD and SGD struggled to learn anything useful.
