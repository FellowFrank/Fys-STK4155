...
\begin{figure}[htbp]
    ...
    \caption{Sample misclassifications by the FFNN with PCA 95\%. The model frequently misclassifies textile as metal due to chromatic similarities.}
...
\begin{figure}[htbp]
    ...
    \caption{Comparative Prediction Analysis. The CNN correctly identifies the paper sample based on edge geometry. The FFNN (PCA) misclassifies it as plastic, as the dimensionality reduction blurred the fibrous texture into a smooth statistical manifold.}
...
While our results show that the CNN is the better model for this task, there are several important steps needed to make this system ready for the real world.

\begin{itemize}
    \item \textbf{Hyperparameter Optimization on Augmented Data:} 
    In this project, we found the best model settings using the original dataset. 
    However, because the augmented dataset is more complex (containing rotations and flips), the optimal settings might change. 

    \item \textbf{Transition to Object Detection:} 
    Currently, our model classifies a single, pre-cropped image. 
    To be useful in a recycling plant, the system needs to process a video feed of a conveyor belt. 
    We need to transition to Object Detection models that can find and "box" multiple items at once.

    \item \textbf{Data Expansion and Localization:} 
    Gathering more training data usually improves performance. 
    Additionally, waste packaging looks different in different countries. 
    Collecting specific data from the local region where the system will be deployed would help the model recognize local brands and packaging types more accurately.
\end{itemize}
